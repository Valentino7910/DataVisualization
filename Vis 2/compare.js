...
function renderGroupBar(deathData, country1, country2, year) {
  ...
  const data1 = deathData
    .filter((item) => item.Country === country1 || item.Country === country2)
    .filter((d) => d.Year === year && d.Gender === "Total" && d.Age === "Total")
    .map((item) => {
  ...
  const data = data1.reduce((acc, current) => {
    const x = acc.find(
      (item) => item.WEEK === current.WEEK && item.Country === current.Country
    );
    if (!x) {
...
function renderGroupLine(deathData, country1, country2, year) {
  ...
  if (!year) {
    ...
    data = data1.filter(
      (d) =>
        d.time === 2020 || d.time === 2021 || d.time === 2022 || d.time === 2023
    );
  } else {
    const data1 = deathData
      .filter(
...
function renderGroupBarAndLine(deathData, country1, country2, year, type) {
  if (trendFlag === null) {
